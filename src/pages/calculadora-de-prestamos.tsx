...
const Page = ({ state, actions, libraries }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const duration = periodType ? Number(data.period) : Number(data.period) * 12 //Se transforma a meses
  ...
  return true ? (
            ...
            {errorMessages.length > 0 && (
              <ErrorMessages>
                {errorMessages.map((item, index) => {
                  return <ErrorMessage key={index}>{item.message}</ErrorMessage>
                })}
              </ErrorMessages>
            )}
    ...
    <MessageBox>
      <MessageTitle color={state.theme.colors.primary.base}>
        Mensaje enviado
      </MessageTitle>
      <MessageText color={state.theme.colors.primary.base}>
